...
#define RWDATA
#else
//#define ROCODE __attribute__((section(".irom1.text")))
//#define RODATA __attribute__((section(".irom1d.text"))) STORE_ATTR
#ifndef BOOT_IMAGE
...
#define FILEINFO_FILENAME_SIZE    (28)
...
extern void update_screen(uint32_t y1, uint32_t y2, void (*render)(uint16_t *buf));
...
#define RAM_MEM_BLOCK1_SIZE     (40 * 1024)
#define RAM_MEM_BLOCK2_SIZE     (12 * 1024)
...
typedef struct {
  ...
  uint32_t (*GetMsTicks)();
  void *(*GetMemBlock)(uint32_t i);
  uint32_t (*GetMemSize)(uint32_t i);
  uint32_t (*GetFreeMem)();
  ...
  uint32_t (*PlayAudio)(uint32_t rate);
  void (*StopAudio)();
...
#define FLASH_ADDR(x)   ((void *)(flashmem + x))
#else
#define FLASH_ADDR(x)   ((void *)(0x40200000 + x))
#endif
