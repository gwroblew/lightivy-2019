...
static unsigned int buffer[I2SDMABUFCNT * I2SDMABUFLEN];
//Pointer to the I2S DMA buffer data
static unsigned int *i2sBuf[I2SDMABUFCNT];
//I2S DMA buffer descriptors
static struct dma_desc i2sBufDesc[I2SDMABUFCNT];
...
unsigned int * ROCODE i2sGetNextBuffer() {
  int next = (lastidx + 1) % I2SDMABUFCNT;
  if (next == dmaidx) {
    return NULL;
  }
  lastidx = next;
...
void ROCODE i2sInit(int rate, int lockBits) {
	int x, y;
