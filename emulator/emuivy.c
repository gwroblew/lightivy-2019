...
uint32_t play_audio(uint32_t rate) {
...
void update_screen16(uint16_t *fb, uint32_t y1, uint32_t y2, int clear) {
  ...
  SDL_Rect dstrect = { 0, y1 * SCREEN_SCALE, SCREEN_WIDTH * SCREEN_SCALE, (y2 + 1) * SCREEN_SCALE - 1 };
  ...
  fpscnt++;
  if (clear)
    memset(fb, 0, SCREEN_HEIGHT * SCREEN_WIDTH * 2);
}
...
void update_screen8(uint8_t *fb, uint16_t *pb, uint32_t y1, uint32_t y2, int clear) {
  ...
  fpscnt++;
  if (clear)
    memset(fb + y1 * SCREEN_WIDTH, 0, (y2 - y1 + 1) * SCREEN_WIDTH);
}
...
uint8_t memblock1[RAM_MEM_BLOCK1_SIZE];
uint8_t memblock2[RAM_MEM_BLOCK2_SIZE];
uint8_t flashmem[4*1024*1024];
uint8_t spiram[8*1024*1024];

void *get_mem_block(uint32_t i){
  if (i == 0)
    return memblock1;
  if (i == 1)
    return memblock2;
...
uint32_t get_mem_size(uint32_t i){
  if (i == 0)
...
int wifi_send_udp(void *data, uint32_t size) {
  ...
  udppack->len = size;
  return !SDLNet_UDP_Send(udpsock, -1, udppack);
}
