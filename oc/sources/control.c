...
uint32_t get_file_checksum(char *fname) {
  if (gsys->FileOpen(fname, FILE_MODE_READ) != 0)
    return 0;
  uint32_t *buf = (uint32_t *)gsys->GetMemBlock(0);
  uint32_t bufsize = gsys->GetMemSize(0);
  uint32_t br, chk = 0;
...
uint32_t control_deferred_connect(OCDEFER *def) {
  if (state == CONTROL_STATE_IDLE) {
    gsys->WifiConnectTCP(0x7F01A8C0, 7070);
  }
  return DEFER_RESULT_OK;
}

uint32_t control_deferred_close(OCDEFER *def) {
...
static void send_file_chunk() {
  uint32_t br = 0;
  void *data = gsys->GetMemBlock(0);
  uint32_t size = gsys->GetMemSize(0);
...
static void read_sd_dir(char *path) {
  FILEINFO *fi = (FILEINFO *)gsys->GetMemBlock(0);
